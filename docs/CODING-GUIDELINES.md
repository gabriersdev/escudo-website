...
- **Uso do Moment.js:** Para toda manipulação, validação e ordenação de datas, deve-se utilizar exclusivamente a biblioteca `moment` em detrimento de objetos globais `Date` do JavaScript.
- **Localização Estrita:** Deve-se sempre instanciar e aplicar a configuração de idioma local antes do processamento, carregando `moment.locale(appConfigs.locale)` (oriundo de `resources/resources.ts`) para assegurar conformidade e precisão das datas em todo o ecossistema.

## Estimativa de Tempo de Leitura

- **Cálculo Autônomo:** O tempo de leitura (`readTime`) das postagens (`.mdx`) deve ser derivado e calculado de forma autônoma utilizando o volume do conteúdo textual cruzado contra uma média pré-fixada de 200 palavras por minuto.
- **Máscara Textual (i18n):** A estrutura que renderiza essa medição para o usuário deve referenciar objetos de máscara no dicionário (`resources/dictionary.ts`, ex: `{{time}} min de leitura`). Isso garante que componentes não engessem o código com pluralizações e sufixos codificados, preservando a interoperabilidade entre idiomas.
