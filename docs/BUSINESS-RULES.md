...
## Inscrição na Newsletter (Segurança)
Toda tentativa de inscrição na newsletter deve satisfazer três regras fundamentais de segurança e negócio para ser aprovada:
1. **Validação Estrutural (Front-end)**: O e-mail deve passar por verificação de formato (Expressão Regular) antes de qualquer requisição de rede, economizando recursos de servidor.
2. **Honeypot Intacto**: A armadilha de formulário (*honeypot input*, tipicamente o campo *company*) não pode conter NENHUM valor. Bots automatizados reprovarão automaticamente ao tentar submeter o formulário massivamente.
3. **Assinatura de Transação Válida (Back-end)**: Nenhuma inscrição será encaminhada ao serviço final sem que o payload contenha o token público. O back-end deve ser capaz de decriptografar este token usando a chave simétrica privada e extrair exatamente a frase de validação acordada no ambiente de sistema (`escudo_newsletter_valid_transaction`). Falhas resultam em negação explícita sem notificar o invasor dos motivos arquiteturais (retorno de *Forbidden* genérico).

## Componentes Interativos em Conteúdos (MDX)
