...
- `resources/dictionary.ts`: Arquivo centralizado que armazena todos os pequenos textos (strings) da interface de usuário (UI). Desacopla as *hardcoded strings* dos componentes React, garantindo que o sistema possua infraestrutura nativa e imediata para internacionalização e localização (i18n).
  - **Exemplo prático de Internacionalização (Tempo de Leitura):** No *journal-template* original, o tempo de leitura era impresso exibindo a variável de metadados pura do Ghost (ex: `{metadata.readTime}` em `post-card.tsx`), que costuma trazer o texto em inglês embutido (ex: `"2 min read"`). No *escudo-website*, essa formatação foi delegada ao dicionário através da chave `post.readingTime` (onde temos `"{{time}} min para ler"`). Isso garante que os componentes exibam a interface consistentemente no nosso idioma sem engessar a visualização a partir dos dados brutos do *headless* CMS.
